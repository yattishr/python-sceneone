...
# Default to a Gemini Live-capable model for run_live WebSocket sessions.
# Override via GEMINI_MODEL in .env when needed.
LIVE_MODEL = os.getenv("GEMINI_MODEL", "gemini-2.5-flash-native-audio-preview-12-2025")
SCRIPT_EXPORT_DIR = Path("exports/scripts")
SCRIPT_EXPORT_DIR.mkdir(parents=True, exist_ok=True)
...
3. Always align the final ad script to the user's requested duration (10, 20, or 30 seconds).
4. When they say 'Action', deliver a script that sounds like it belongs in a real commercial, not an AI list 
    AND use the 'capture_ad_script' tool with product_name, final_script, and duration_seconds.
5. If the user has not specified a duration, default to 10 seconds.
...
def _safe_product_name(product_name: str) -> str:
    cleaned = re.sub(r"[^a-zA-Z0-9_-]+", "_", product_name.strip().lower()).strip("_")
    return cleaned or "untitled_product"

def capture_ad_script(product_name: str, final_script: str, duration_seconds: int = 10):
    """
    ...
    Args:
        ...
        final_script: The final script generated by the director
        duration_seconds: The target ad duration. Allowed values: 10, 20, 30.
    """
    duration_seconds = int(duration_seconds)
    if duration_seconds not in {10, 20, 30}:
        raise ValueError("duration_seconds must be one of: 10, 20, 30")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"script_{_safe_product_name(product_name)}_{timestamp}.txt"
    file_path = SCRIPT_EXPORT_DIR / filename
